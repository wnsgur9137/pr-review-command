...
```
/commit [--no-gitmoji] [--gitmoji] [--no-coauthor] [--all] [--amend] [--no-split] [--yes]
```
...
/commit --all --yes      # 전체 스테이징 + 미리보기 생략하고 바로 커밋
...
| 옵션 | 설명 |
|------|------|
| `--no-gitmoji` | gitmoji를 절대 붙이지 않음 (최우선) |
| `--gitmoji` | 컨벤션이 이모지를 안 써도 gitmoji 강제 |
| `--no-coauthor` | `Co-Authored-By: Claude ...` 푸터 제거 (기본은 유지) |
| `--all` | 스테이징된 변경이 없을 때 확인 없이 `git add -A` 후 진행 |
| `--amend` | 새 커밋 대신 직전 커밋 수정 (푸시된 경우 경고) |
| `--no-split` | 변경이 섞여 있어도 커밋 분리를 제안하지 않고 단일 커밋 |
