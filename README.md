...
>
> **필수 조건**: **GitHub MCP 서버**가 프로젝트의 `.mcp.json`에 설정되어 있어야 동작합니다. 아래 [설정](#설정) 섹션을 먼저 확인하세요.
...
`/review`의 코멘트 게시(Step 6.3)는 GitHub MCP의 pending review 워크플로우를 사용합니다:
...
> **결론**: PR 데이터 조회는 `gh` CLI로 대체 가능하지만, **인라인 코멘트를 리뷰로 묶어서 게시하는 기능은 GitHub MCP가 필수**입니다. `/apply-review`는 이 도구를 사용하지 않으므로 `gh` CLI만으로도 동작 가능합니다.
...
| `/review <PR_URL>` | PR 코드 리뷰 수행 및 GitHub 인라인 코멘트 게시 |
| `/review --help` | `/review` 도움말 표시 |
...
/review https://github.com/owner/repo/pull/123 --mode=strict
/review https://github.com/owner/repo/pull/123 --focus=security,performance
```
...
설치 후 별도 설정 없이 `/review` 실행 시 자동 감지됩니다.

### 토큰 최적화

`/review` 커맨드는 토큰 효율성을 위해 `~/.claude/agents/`에 정의된 전용 서브에이전트를 활용하여 모델을 강제 분기합니다.
...
| 항목 | 필수 여부 | 설명 |
|------|----------|------|
| **GitHub MCP 서버** | 필수 | PR 정보 수집, 코멘트 게시에 사용. 프로젝트 `.mcp.json`에 설정 |
| **`GITHUB_PERSONAL_ACCESS_TOKEN`** | 필수 | GitHub API 인증용 환경변수 |
| **`~/.claude/commands/`** | 필수 | 글로벌 커맨드 정의 파일 (`create-pr.md`, `review.md`, `apply-review.md`) |
| **`~/.claude/templates/`** | 필수 | 출력 템플릿 및 플랫폼별 리뷰 기준 |
| **oh-my-claudecode (OMC)** | 불필요 | `/review`, `/apply-review`는 OMC와 독립적으로 동작. OMC 설치 여부와 무관하게 사용 가능 |
| **code-review-graph** | 선택 | 설치+그래프 빌드 시 AST 기반 정밀 분석 자동 활용. 미설치 시 기존 Grep/Glob 방식으로 폴백 |

> **OMC와의 관계**: OMC는 Claude Code의 멀티 에이전트 오케스트레이션 플러그인으로, `/review` 커맨드와는 완전히 별개입니다. OMC가 제공하는 `/oh-my-claudecode:code-review`는 OMC 자체의 코드 리뷰 기능이며, 이 프로젝트의 `/review`와는 다른 도구입니다.
>
> **code-review-graph와의 관계**: code-review-graph가 Claude Code 플러그인으로 설치되고 MCP 서버가 연결되면, `/review` 실행 시 자동으로 감지하여 blast radius 분석, 테스트 커버리지 확인, 정밀 호출자 추적 등을 수행합니다. 별도 설정 없이 "있으면 활용, 없으면 기존 방식" 으로 동작합니다.
...
1. **GitHub MCP 서버**가 `.mcp.json`에 설정되어 있어야 합니다
...
│   │   ├── general.md
│   │   └── README.md               # 가이드 문서
│   ├── review/                      # /review 출력 형식 (선택적 커스터마이징)
│   │   └── templates.md
...
- **리뷰 출력 형식**: `.claude/templates/review/templates.md`에서 `/review` 커맨드의 출력 템플릿을 편집
